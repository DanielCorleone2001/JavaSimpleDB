    ...
    public List<LogicalJoinNode> orderJoins(Map<String, TableStats> stats, Map<String, Double> filterSelectivities, boolean explain) throws ParsingException {
        ...
        for (int i = 1; i <= joins.size(); i++) {
            ...
            for (Set<LogicalJoinNode> set : subsets) {
                CostCard bestCostCart = new CostCard();
                bestCostCart.cost = Double.MAX_VALUE;

                for (LogicalJoinNode node : set) {
                    CostCard curCostCard = computeCostAndCardOfSubplan(stats, filterSelectivities, node, set, bestCostCart.cost, planCache);

                    if (curCostCard != null && curCostCard.cost < bestCostCart.cost) bestCostCart = curCostCard;
                }

                planCache.addPlan(set, bestCostCart.cost, bestCostCart.card, bestCostCart.plan);
            }
